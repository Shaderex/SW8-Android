...
public class BarometerSensorProvider extends SensorProvider<List<Float>> {
  ...
  private class RetrieveBarometerDataCallable extends RetrieveSensorDataCallable {

    public RetrieveBarometerDataCallable(final long sampleDuration, final int measurementFrequency) {
      super(sampleDuration, measurementFrequency);
    }

    // Listeners used when we have one measurement from each sensor
    private SensorEventListener barometerEventListener;
    ...
    public List<Float> call() throws InterruptedException {
      ...
      barometerEventListener = new SensorEventListener() {
        @Override
  ...
  protected RetrieveSensorDataCallable createCallable(final long sampleDuration, final int measurementFrequency) {
    return new RetrieveBarometerDataCallable(sampleDuration, measurementFrequency);
  }
}
